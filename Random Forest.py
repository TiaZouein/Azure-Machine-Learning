...
def parse_args():
    ...
    parser.add_argument('--max_samples', type=int, default=0.5,
    ...
    parser.add_argument('--n_features_to_select', type=str, default = 'auto',
                            help='sfs')
    ...
    parser.add_argument('--scoring', type=str, default = "recall",
                            help='sfs')
...
def main(args):
    ...
    mlflow.log_param("random_forest_model", "RandomForestClassifier")
    ...
    mlflow.log_param('n_splits', int(args.n_splits))
    mlflow.log_param('shuffle', bool(args.shuffle))
    mlflow.log_param('n_features_to_select', str(args.n_features_to_select))
    mlflow.log_param('tol', float(args.tol))
    mlflow.log_param('direction', str(args.direction))
    mlflow.log_param('scoring', str(args.scoring))
    ...
    print("Y_train_resampled_Count:", np.bincount(Y_train_resampled))
    print(sorted(Counter(Y_train_resampled).items()))


    random_forest_model = RandomForestClassifier(n_estimators= args.n_estimators,
    ...
    randomforest_model = random_forest_model.fit(X_train_resampled, Y_train_resampled)
    print("predicting values")
    y_randomforest = random_forest_model.predict(X_test)
    ...
    accuracyscore = metrics.accuracy_score(Y_test,y_randomforest)*100
    f1score = metrics.f1_score(Y_test,y_randomforest)*100
    recallscore = metrics.recall_score(Y_test,y_randomforest)*100
    specificityscore = metrics.recall_score(Y_test, y_randomforest, pos_label=0)*100
    precisionscore = metrics.precision_score(Y_test,y_randomforest)*100
    ...
    cm_display.figure_.savefig('confusion_matrix_RF.png')
    mlflow.log_artifact("confusion_matrix_RF.png")

    ######## COURBE DE ROC
    # calculate the fpr and tpr for all thresholds of the classification
    prob = random_forest_model.predict_proba(X_test)
    pred = prob[:,1]
    ...
    registered_model_name="sklearn_RF_model"

    print("Registering the model via MLFlow")
    mlflow.sklearn.log_model(
        sk_model=random_forest_model_sampled,
...
if __name__ == "__main__":
    ...
    lines = [
        ...
        f"n_features_to_select: {args.n_features_to_select}" 
        f"tol: {args.tol}" 
